...
void InitGUI()
{
	...
	TwAddVarRW(bar, "Frontface CW?", TW_TYPE_BOOLCPP, &bFrontFace, "");
...
void CreateGeometry()
{
	...
	for(float angle = 0.0f; angle < (2.0f*GL_PI); angle += (GL_PI/8.0f))
	{
		...
		// Spezifiziere den naechsten Vertex des Triangle_Fans
		//boden.Vertex3f(x, y,0);v - darf nicht sein, da sonst der Boden nicht sichtbar ist
		boden.Vertex3f(y, x,0);
	}
...
void RenderScene(void)
{
	...
	// Setze die Vorderseite von Polygonen auf CW oder CCW
	if(bFrontFace)
		glFrontFace(GL_CW);
	else
		glFrontFace(GL_CCW);
